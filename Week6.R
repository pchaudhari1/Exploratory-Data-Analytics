...
# Consider Mary's age as x
x <- NA
# Consider Johns age as x
y <- NA
# Check is Mary and John are of same age.
x == y
...
#using desc() to order column in descending order
...
# Check when first and last flight leave each day
not_cancelled %>% 
  group_by(year, month, day) %>% 
  summarise(
    first = min(dep_time),
...
# Check destinations with most carriers
not_cancelled %>% 
  group_by(dest) %>% 
...
# Check flights leaving before 5am
not_cancelled %>% 
  group_by(year, month, day) %>% 
  summarise(n_early = sum(dep_time < 500))

# Check proportions of flights delayed by moe than an hour
not_cancelled %>% 
  group_by(year, month, day) %>% 
