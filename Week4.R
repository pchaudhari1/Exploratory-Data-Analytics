...
# Create a matrix with integers 1:10 having 5 rows and 2 columns
matrix(data = 1:10, nrow = 5, ncol = 2)


# Having 2 rows and 5 columns
matrix(data = 1:10, nrow = 2, ncol = 5)

# Having 2 rows and 5 columns, but fill by row instead of columns
...
# Creating a dataframe of the survey data WITHOUT factors
survey <- data.frame("index" = c(1, 2, 3, 4, 5),
                     "sex" = c("m", "m", "m", "f", "f"),
                     "age" = c(99, 46, 23, 54, 23),
                     stringsAsFactors = FALSE)

# Print the result of survey
survey

# Look at the structure. it does not contain any factors.
str(survey)
...
# $ returns the len column of ToothGrowth
ToothGrowth$len
...
# Len and supp columns of ToothGrowth. Returns top 6 rows
head(ToothGrowth[c("len", "supp")])

# Create a new dataframe named survey
survey <- data.frame("index" = c(1, 2, 3, 4, 5),
...
# Adding a new column sex to survey dataframe
survey$sex <- c("m", "m", "f", "f", "m")

# Print survey with new sex column
survey  


# Change name of 1st column of dataframe to "a"
names(df)[1] <- "a"
...
# Change the column name age to age.years
...
# Return first 5 rows and 2nd column
df[1:5, 2]
...
# Create a new df with the rows of ToothGrowth having supp equals VC
ToothGrowth.VC <- ToothGrowth[ToothGrowth$supp == "VC", ]

# Create a new df with only the rows of ToothGrowth having supp equals OJ and dose < 1
...
# Mean tooth length og Guniea pigs given OJ
# Create a subsetted dataframe called oj
oj <- subset(x = ToothGrowth = supp == "OJ")

# Mean of len columnn from oj
mean(oj$len)

# Using logial indexing
j <- ToothGrowth[ToothGrowth$supp == "OJ",]

# Mean
mean(oj$len)

# Line by line by referring to column vectors
mean(ToothGrowth$len[ToothGrowth$supp == "OJ"])

# with()
#Create datafreame health
...
#Naming vectors during creation
c(x = 1, y = 2, z = 4)

# using purrr:set_names() to set names
...
# See implementation of method using getS3method()
